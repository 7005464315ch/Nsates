...
class staffvehicalenController extends Controller
{
    public function staffvehicalen() {
            $datao = array();
        if (session::has('staffaid')) {
            $datao = staff::where('id','=',session::get('staffaid'))->first();
            
        }
        return view('staff.staffvehicalen',compact('datao'));
    }

    public function staffvehicalon(Request $request) {
