...
class fastguestentryController extends Controller
{
    public function fastguestentry() {
          $datao = array();
        if (session::has('fadminid')) {
            $datao = fastadmin::where('id','=',session::get('fadminid'))->first();
            
        }
        return view('fastadmin.fastguestentry',compact('datao'));
    }



     public function fastguesten(Request $request) {
