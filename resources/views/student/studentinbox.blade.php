@extends('student.studentmessage')
   ...
   <style type="text/css">
    ...
    <div class="col-sm-6" id="helpxc" style="float: left;">
             <form action="{{url('studiofencode')}}" method="post">
               @csrf
          ...
          <div class="col-sm-6" id="helpxc" style="float: left;">
             <form action="{{url('studiofendate')}}" method="post">
               @csrf
...
<table class="table table-bordered table-hover">
  ...
  <tbody>
    @foreach($studin as $key=>$studi)
    <tr>
       <td scope="row">{{++$key}}</td>
       <td>{{$studi->entry_code}}</td>
       <td>{{$studi->created_date}}</td>
       <td>{{$studi->student_code}}</td>
       <td>{{$studi->student_name}}</td>
       <td>{{$studi->email}}</td>
       <td>{{$studi->subject}}</td>
        <td><a data-studi_id="{{$studi->id}}" data-entry_code="{{$studi->entry_code}}" data-created_date="{{$studi->created_date}}" data-reference_code="{{$studi->reference_code}}"  data-reference_name="{{$studi->reference_name}}" data-student_code="{{$studi->student_code}}"  data-student_name="{{$studi->student_name}}"  data-email="{{$studi->email}}"   data-subject="{{$studi->subject}}" data-description="{{$studi->description}}"    data-toggle="modal" data-target="#exampleModal-showzc" type="button"class="btn btn-warning btn-sm">Show</a></td>
    </tr>
    @endforeach
  </tbody></table></div>
